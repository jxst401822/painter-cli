...
description: "Generate sugar painting (糖画/糖画) patterns from text prompts. Produces amber-on-black line art in the traditional Chinese sugar painting style. Uses dayin.la AI engine with Volcengine ARK fallback."
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [image-generation, sugar-painting, tanghua, 糖画, creative]
---
...
2. **Volcengine ARK** (fallback) — Uses doubao-seedream-5.0-lite
   ...
   - Requires ark-helper config at ~/.ark-helper/config.yaml
...
python ~/.hermes/skills/creative/sugar-painting-gen/scripts/sugar_painting_gen.py \
...
from sugar_painting_gen import generate
for p in ["龙", "凤凰", "蝴蝶"]:
    generate(prompt=p, output=f"/home/se/sugar_{p}.png", engine="dayinla")
...
- **ARK engine post-processing**: The ARK engine's prompt already instructs the model
  ...
  doesn't look right.
- **ARK engine relies on volcengine-ark-image-gen skill**: The ARK engine reads the
  API key from `~/.ark-helper/config.yaml` — same mechanism as the
  `volcengine-ark-image-gen` skill. See that skill for ARK API details.
